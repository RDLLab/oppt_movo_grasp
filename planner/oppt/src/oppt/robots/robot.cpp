...
bool Robot::initializeViewer_(const std::string &robotName, const std::string &environmentFile)
{
#ifdef USE_RVIZ
    viewer_ = std::make_shared<ViewerPublisher>();
    static_cast<ViewerPublisher*>(viewer_.get())->setupViewer(robotName, environmentFile);
#endif
    return true;
}
                                  ...
                                  RobotEnvironment* robotEnvironment)
{
    ...
    if (oppt::resources::FileExists(pluginFile)) {
        std::string observationPluginPath = oppt::resources::FindFile(pluginFile);
        LOGGING("Observation plugin path: " + observationPluginPath);
    }
    observationPlugin_ =
...
void Robot::setEnvironmentInfo(oppt::EnvironmentInfoSharedPtr& environmentInfo)
{
    ...
    environmentInfo_->getScene()->setCollisionInvariantRobotCollisionObjects(invariantRobotCollisionObjects);
}
...
void Robot::setupViewer(const std::string &robotName, const std::string &environmentFile)
{
#ifdef USE_RVIZ
    if (viewer_) {
        static_cast<ViewerPublisher*>(viewer_.get())->setupViewer(robotName, environmentFile);
