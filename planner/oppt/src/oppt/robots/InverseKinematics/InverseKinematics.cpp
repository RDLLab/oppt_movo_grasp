...
VectorFloat TracIKSolver::endEffectorForceFromTorque(const VectorFloat &currentJointAngles, const VectorFloat &tau) {
	...
	KDL::JntArray tauArr;
	Vectordf tauEigen(chain_->getNrOfJoints());
	...
	Matrixdf jacobianMatrix = Matrixdf::Zero(jacobian_->rows(), jacobian_->columns());
	...
	return toStdVec<FloatType>(jacobianMatrix.transpose().inverse() * tauEigen);
	KDL::Twist t;
