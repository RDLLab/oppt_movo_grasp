...
void SceneImpl::changeBodyPoses(const VectorString& names, const std::vector<geometric::Pose>& poses, bool executeCallback) {
    ...
    for (size_t i = 0; i != names.size(); ++i) {
        ...
        if (body) {
            ...
            body->updateCollisionObject();
            fclCollisionObjectsToUpdate.push_back(body->getOpptCollisionObject()->getCollisionObject());
        }
    }
...
{
    oppt::DiscreteCollisionReport* discreteCollisionReport = static_cast<oppt::DiscreteCollisionReport*>(collisionReport.get());
    ...
    for (size_t i = 0; i != collisionData.o1.size(); ++i) {
        ...
        discreteCollisionReport->collisionPairs.push_back(
            std::pair<std::string, std::string>(((OpptCollisionObjectData *)(collisionData.o2[i]->getUserData()))->opptCollisionObject->getName(),
                                                ((OpptCollisionObjectData *)(collisionData.o1[i]->getUserData()))->opptCollisionObject->getName()));        
    }
}
    ...
    for (size_t i = 0; i < bodies_.size(); i++) {
        ...
        for (size_t j = 0; j < continuousCollisionReport->tfGoal.size(); j++) {
            ...
            if (result.is_collide) {
                ...
                continuousCollisionReport->collisionPairs.push_back(
                    std::pair<std::string, std::string>({robotCollisionObjects_[j]->getName(), bodies_[i]->getOpptCollisionObject()->getName()}));
        ...
        for (size_t j = 0; j < collisionReport->robotCollisionObjectsStart.size(); j++) {
            ...
            if (result.is_collide) {
                ...
                collisionReport->collisionPairs.push_back(
                    std::pair<std::string, std::string>({collisionReport->robotCollisionObjectsGoal[j]->getName(),
                                                        bodies_[i]->getOpptCollisionObject()->getName()
                                                        }));
