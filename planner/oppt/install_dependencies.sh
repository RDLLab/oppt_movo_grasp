...
install_ros() {
    ...
    then
       ...
       if [[ $rv != *"kdl-parser"* ]]; then 
          ...
          ROS_PACKAGES="$ROS_PACKAGES ros-$ROS_DISTR-kdl-parser"
       fi
       rv=$(rospack find trac-ik) 
       if [[ $rv != *"kdl-parser"* ]]; then 
          echo "Install ros-$ROS_DISTR-trac-ik"
          ROS_PACKAGES="$ROS_PACKAGES ros-$ROS_DISTR-trac-ik ros-$ROS_DISTR-trac-ik-lib"
       fi   
    else
       ROS_PACKAGES="ros-$ROS_DISTR-ros-base ros-$ROS_DISTR-rviz ros-$ROS_DISTR-kdl-parser ros-$ROS_DISTR-trac-ik ros-$ROS_DISTR-trac-ik-lib"
    fi
