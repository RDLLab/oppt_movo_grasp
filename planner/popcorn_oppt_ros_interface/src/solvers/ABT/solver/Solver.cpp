...
#include "ABT/robotModel/solver/StateInfo.hpp"
...
/* ------------------ Display methods  ------------------- */
void Solver::printBelief(BeliefNode* belief, std::ostream& os, int counter, bool wrtiteToCsv)
{   std::ofstream myfile;
    if (wrtiteToCsv)
    {
        myfile.open("/home/movodev/trial.csv", std::ios::out | std::ios::app);
    }
    ...
    for (auto it = actionValues.rbegin(); it != actionValues.rend(); it++) {
        ...
        if (wrtiteToCsv)
        {
            myfile<<counter<<";"<<it->first<<";"<<*it->second.first->getAction()<<";"<<it->second.first->getVisitCount()<<endl;
        }
    }
    if (wrtiteToCsv)
    {
